...
class GraphDataset(Dataset):
    def __init__(self, root, transform=None, pre_transform=None, n_particles=-1,
                 bb=0, n_events=-1, n_proc=1, R=1.0, n_events_merge=100, features='xyz'):
        """
    ...
    def process_one_chunk(self, raw_path, k):
        ...
        datas = []
        for particles, n_particles, jet_mass, jet_px, jet_py, jet_pz, jet_e, signal_bit, row in tqdm(part_gen, desc=f'Process: {k}', position=k, total=2*len(df)):
